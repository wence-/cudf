...
struct ungrouped {
  ...
  [[nodiscard]] __device__ constexpr bool is_null(cudf::size_type) const noexcept { return false; }

  [[nodiscard]] __device__ constexpr bool null_start(cudf::size_type,
                                                     cudf::size_type) const noexcept
  {
    return 0;
  }

  [[nodiscard]] __device__ constexpr bool null_end(cudf::size_type, cudf::size_type) const noexcept
  {
    return 0;
  }
};
...
struct ungrouped_with_nulls {
  ...
  [[nodiscard]] __device__ constexpr bool is_null(cudf::size_type i) const noexcept
  {
    return (nulls_at_start && i < null_count) || (!nulls_at_start && i >= num_rows - null_count);
  }

  [[nodiscard]] __device__ constexpr bool null_start(cudf::size_type,
                                                     cudf::size_type) const noexcept
  {
    return nulls_at_start ? 0 : num_rows - null_count;
  }

  [[nodiscard]] __device__ constexpr bool null_end(cudf::size_type, cudf::size_type) const noexcept
  {
    return nulls_at_start ? null_count : num_rows;
  }
};
...
struct grouped {
  ...
  // Should never be called because is_null always returns false
...
struct grouped_with_nulls {
  ...
  [[nodiscard]] __device__ bool is_null(cudf::size_type i) const noexcept
  {
    return orderby.is_null_nocheck(i);
  }

  [[nodiscard]] __device__ bool null_start(cudf::size_type start, cudf::size_type end) const
  {
    if (nulls_at_start) {
      return start;
    } else {
      return *thrust::partition_point(
        thrust::seq,
        thrust::make_counting_iterator(start),
        thrust::make_counting_iterator(end),
        [&] __device__(cudf::size_type i) { return orderby.is_valid_nocheck(i); });
    }
  }

  [[nodiscard]] __device__ bool null_end(cudf::size_type start, cudf::size_type end) const
  {
    if (nulls_at_start) {
      return *thrust::partition_point(
        thrust::seq,
        thrust::make_counting_iterator(start),
        thrust::make_counting_iterator(end),
        [&] __device__(size_type i) { return orderby.is_null_nocheck(i); });
    } else {
      return end;
    }
  }
...
struct range_window_clamper {
  ...
  struct distance_kernel {
    ...
    [[nodiscard]] __device__ size_type operator()(size_type i) const
    {
      ...
      if constexpr (Direction == direction::PRECEDING) {
        if constexpr (WindowType == window_type::UNBOUNDED) { return i - start + 1; }
        if (groups.is_null(i)) { return i - groups.null_start(start, end) + 1; }
        ...
        if constexpr (WindowType == window_type::UNBOUNDED) { return end - i - 1; }
        if (groups.is_null(i)) { return groups.null_end(start, end) - i - 1; }
        if constexpr (WindowType == window_type::CURRENT_ROW) {
    ...
    if (group_keys.num_columns() == 0) {
      ...
      if (orderby.has_nulls()) {
        copy_n(distance_kernel<grouped, OrderbyT, DeltaT>{
          grouped_with_nulls{labels.data(), offsets.data(), *d_orderby, nulls_at_start},
          d_row_delta,
          d_begin,
          d_end});
      } else {
  ...
  template <typename OrderbyT,
            CUDF_ENABLE_IF(cudf::is_fixed_point<OrderbyT>() ||
                           (cudf::is_index_type<OrderbyT>() && !cudf::is_unsigned<OrderbyT>()))>
  [[nodiscard]] std::unique_ptr<column> operator()(table_view const& group_keys,
                                                   column_view const& orderby,
                                                   bool nulls_at_start,
                                                   scalar const* row_delta,
  ...
  template <typename OrderbyT,
            CUDF_ENABLE_IF(!(cudf::is_timestamp<OrderbyT>() || cudf::is_fixed_point<OrderbyT>() ||
                             (cudf::is_index_type<OrderbyT>() && !cudf::is_unsigned<OrderbyT>())))>
