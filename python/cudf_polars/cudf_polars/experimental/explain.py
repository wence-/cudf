...
from cudf_polars.experimental.tablestats import collect_statistics
...
def explain_query(
    ...
    stats = collect_statistics(ir)
    if physical:
        # TODO: plumb stats through lowering
        lowered_ir, partition_info = lower_ir_graph(ir, config)
        return _repr_ir_tree(lowered_ir, partition_info)
    else:
        return _repr_ir_tree(ir, row_estimates=stats.estimated_row_counts)
...
def _repr_ir_tree(
    ...
    *,
    offset: str = "",
    row_estimates: dict[IR, int] | None = None,
) -> str:
    header = _repr_ir(ir, offset=offset)
    count = partition_info[ir].count if partition_info else None
    row_count = f"~{row_estimates[ir]}" if row_estimates is not None else "unknown"
    ...
    children_strs = [
        _repr_ir_tree(
            child, partition_info, offset=offset + "  ", row_estimates=row_estimates
        )
