...
def execute_plan(visitor: Visitor) -> DataFrame:
    """
    ...
    """
    return PlanVisitor(visitor, {}, ExprVisitor(visitor))(n=None)
...
def _dataframescan(plan: _ir_nodes.DataFrameScan, visitor: PlanVisitor):
    ...
    context = DataFrame(
        dict(zip(arrow_table.column_names, plc_table.columns(), strict=True))
    )
...
def _select(plan: _ir_nodes.Select, visitor: PlanVisitor):
    ...
    for cse in plan.cse_expr:
        context |= {cse.output_name: visitor.expr_visitor(cse.node, context)}
    return DataFrame(
        {
    ...
    if right_policy is None:
        ...
        if how == "outer" and coalesce_key_columns:
            for name, replacement in zip(
                ...
                left[name] = plc.replace.replace_nulls(left[name], replacement)
...
def _distinct(plan: _ir_nodes.Distinct, visitor: PlanVisitor):
    ...
    keys_sorted = all(sortedness[c] != DataFrame.IsSorted.NOT for c in subset)
    if keys_sorted:
...
def _map_function(plan: _ir_nodes.MapFunction, visitor: PlanVisitor):
    ...
    if typ == "unnest":
        ...
        indices = [i for i, name in enumerate(column_names) if name in subset]
        ...
        if not all(column_names == r.names() for r in rest):
            raise ValueError("DataFrame names must all align in merge_sorted")
        indices = [
